  ...
  return (
    <div key={startup.id} className="relative col-span-2 lg:col-span-1">
        ...
        <div className="text-primary-text flex items-center justify-between">
          <div className="flex items-center justify-center">
            {startup.status === "discontinued" ? (
              <span className="text-xs bg-red-200 font-bold pl-1 rounded-l-full">
                🚫
              </span>
            ) : (
              <span className="text-xs bg-violet-500 font-bold pl-1.5 pr-1 rounded-l-full">
                ₹
              </span>
            )}
            {startup.status === "discontinued" ? (
              <span className="text-xs text-red-700 bg-red-200 rounded-r-lg pr-1 p-[0.055rem]">
                discontinued
              </span>
            ) : (
              <span className="text-xs bg-primary-text text-black rounded-r-lg pl-0.5 pr-0.5 ">
      ...
      <div
        ...
        className={`w-full rounded-md mt-2 p-4 border-[0.5px] ${startup.status === "discontinued" ? 'h-[6rem]' : 'h-[12rem]'}`}
      >
        <div className="flex items-center">
          <div
            ...
            className="w-[3.5rem] h-[3.5rem] p-1 rounded-full border border-dashed mr-2"
          >
            <Image
        ...
        {startup.status !== "discontinued" &&
          startup.show_toggle !== "none" && (
            <span
              ...
              className="mt-2 text-sm markdown_content"
            >
              {startup.status !== "discontinued" &&
                startup.show_toggle !== "none" && (
                  <>
