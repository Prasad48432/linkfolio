...
const CategoryBadge = ({ object,size }: { object: any; size: string }) => { 
    // Find the corresponding status info object
    const statusInfoItem = categoryInfo.find(item => item.status === object.category);
  
    if (statusInfoItem) {
      const { text, icon, color } = statusInfoItem;
      return (
        <span className={`inline-flex items-center ${size === 'sm' ? 'text-[0.4rem] lg:text-[0.5rem]' : 'text-[0.5rem] lg:text-[0.7rem]' } px-0.5 py-[0.0625rem] rounded border ${color}`}>
          {icon} {text}
        </span>
      );
    } else {
